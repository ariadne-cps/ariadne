...
template<class X>
Expansion<X>::Expansion(unsigned int as, unsigned int deg, InitializerList<X> lst)
    : _argument_size(as)
{
    MultiIndex a(as); X x;
    auto iter = lst.begin();
...
template<class X>
Expansion<X>::Expansion(unsigned int as, InitializerList< std::pair<InitializerList<int>,X> > lst)
    : _argument_size(as)
{
    MultiIndex a;
    X x;
