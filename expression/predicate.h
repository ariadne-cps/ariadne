...
class PredicateInterface {
  public:
    ...
    virtual ValidatedKleenean evaluate(const Vector<Float64>& x) const = 0;
    virtual ValidatedKleenean evaluate(const Vector<ValidatedNumericType>& x) const = 0;
};
    ...
    : public PredicateInterface
{
  ...
  public:
    ...
    ValidatedKleenean evaluate(const Vector<Float64>& x) const {
        Float64 value=_expression.evaluate(x)*_sign;
        if(value<0) { return true; }
        else if(value>0) { return false; }
        else { return indeterminate; } }
    ValidatedKleenean evaluate(const Vector<ValidatedNumericType>& x) const {
    ...
    : public PredicateInterface
{
  public:
    ...
    virtual ValidatedKleenean evaluate(const Vector<Float64>& x) const;
    virtual ValidatedKleenean evaluate(const Vector<ValidatedNumericType>& x) const;
