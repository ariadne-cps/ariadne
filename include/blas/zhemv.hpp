...
#ifndef __BLAS_ZHEMV_HPP__
#define __BLAS_ZHEMV_HPP__

#include "blas.hpp"

template<typename real>
void
BLAS::hemv (const enum ORDER order, const enum UPLO Uplo,
             const int N, const complex<real> alpha, const complex<real> *A, const int lda,
             const complex<real> *X, const int incX, const complex<real> beta, complex<real> *Y,
             const int incY)
{
{
  ...
  if ((order == RowMajor && Uplo == Upper)
    ...
    for (i = 0; i < N; i++) {
      ...
      real Aii_real = (((const real *) A)[2*(lda * i + i)]);
      (((real *) Y)[2*(iy)]) += temp1_real * Aii_real;
      (((real *) Y)[2*(iy)+1]) += temp1_imag * Aii_real;
      for (j = j_min; j < j_max; j++) {
        real Aij_real = (((const real *) A)[2*(lda * i + j)]);
        real Aij_imag = conj * (((const real *) A)[2*(lda * i + j)+1]);
    ...
    xerbla(0, "source_hemv.h", "unrecognized operation");;
  }
}
}

#endif // __BLAS_ZHEMV_HPP__
