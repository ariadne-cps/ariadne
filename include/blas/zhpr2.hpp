#ifndef __BLAS_ZHPR2_HPP__
#define __BLAS_ZHPR2_HPP__

#include "blas.hpp"

template<typename real>
void
BLAS::hpr2 (const enum ORDER order, const enum UPLO Uplo,
...
{
{
  ...
  if ((order == RowMajor && Uplo == Upper)
    ...
    for (i = 0; i < N; i++) {
      ...
      int jx = ix + incX;
      int jy = iy + incY;
      (((real *) Ap)[2*(((((((i)-1)+1)*(2*(N)-((i)-1)))/2)+(i)-(i)))]) += 2 * (tmp1_real * Yi_real + tmp1_imag * Yi_imag);
      ...
      for (j = i + 1; j < N; j++) {
        ...
        (((real *) Ap)[2*(((((((i)-1)+1)*(2*(N)-((i)-1)))/2)+(j)-(i)))]) += ((tmp1_real * Yj_real + tmp1_imag * Yj_imag)
                                    + (tmp2_real * Xj_real + tmp2_imag * Xj_imag));
        (((real *) Ap)[2*(((((((i)-1)+1)*(2*(N)-((i)-1)))/2)+(j)-(i)))+1]) +=
    ...
    for (i = 0; i < N; i++) {
      ...
      for (j = 0; j < i; j++) {
        ...
        const real Yj_real = (((const real *) Y)[2*(jy)]);
        const real Yj_imag = (((const real *) Y)[2*(jy)+1]);
        (((real *) Ap)[2*((((i)*((i)+1))/2 + (j)))]) += ((tmp1_real * Yj_real + tmp1_imag * Yj_imag)
                                    + (tmp2_real * Xj_real + tmp2_imag * Xj_imag));
        (((real *) Ap)[2*((((i)*((i)+1))/2 + (j)))+1]) +=
            conj * ((tmp1_imag * Yj_real - tmp1_real * Yj_imag) +
                    (tmp2_imag * Xj_real - tmp2_real * Xj_imag));
        jx += incX;
        jy += incY;
      }
      (((real *) Ap)[2*((((i)*((i)+1))/2 + (i)))]) += 2 * (tmp1_real * Yi_real + tmp1_imag * Yi_imag);
      (((real *) Ap)[2*((((i)*((i)+1))/2 + (i)))+1]) = 0;
      ix += incX;
      iy += incY;
    }
  } else {
    xerbla(0, "source_hpr2.h", "unrecognized operation");;
  }
}
}

#endif // __BLAS_ZHPR2_HPP__
