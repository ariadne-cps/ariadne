...
Evaluation::ConstraintHybridEvolverPlugin<R>::subdivide(const timed_set_type& timed_set) const
{
  ...
  {
    result.push_back(timed_set_type(timed_set.time(),timed_set.steps(),timed_set.discrete_state(), Zonotope<I>(*iter)));
  }
  return result;
}
...
Evaluation::ConstraintHybridEvolverPlugin<R>::discrete_step(const transition_type& transition,
...
template<class R>
typename Evaluation::ConstraintHybridEvolverPlugin<R>::timed_set_type
Evaluation::ConstraintHybridEvolverPlugin<R>::integration_step(const mode_type& mode,
                                                         ...
                                                         const time_type& time_step) const
{
  ...
  return timed_set_type(initial_set.time()+time_step,initial_set.steps(),initial_set.discrete_state(),continuous_state_set);
}
                                                               ...
                                                               const bounding_box_type& bounding_box) const
{
  ...
  ARIADNE_LOG(2,"HybridEvolverPlugin::forced_jump(...)\n");
  ARIADNE_LOG(3,"  transition_set="<<transition<<"\n");
  ARIADNE_LOG(3,"  initial_set="<<initial_set<<"\n");
  ARIADNE_LOG(3,"  bounding_box="<<bounding_box<<"\n");
...
Evaluation::ConstraintHybridEvolverPlugin<R>::unforced_jump_step(const transition_type& transition,
                                                                 const timed_set_type& initial_set,
                                                                 const bounding_box_type& flow_bounding_box,
                                                                 const time_type& maximum_time) const
{
  throw NotImplemented(__PRETTY_FUNCTION__);
  /*
  ARIADNE_LOG(2,"HybridEvolverPlugin::unforced_jump(...)\n");
...
Evaluation::ConstraintHybridEvolverPlugin<R>::upper_evolution_step(const System::ConstraintHybridAutomaton<R>& automaton, 
                                                             ...
                                                             time_type& time_step) const
{
  ARIADNE_LOG(2,"HybridEvolverPlugin::upper_evolution_step(...)\n");
  ARIADNE_LOG(3,"  initial_set="<<initial_set<<"\n");  
  ...
  const time_model_type& time = initial_set.time();
      ...
      constraint_iter != activations.end(); ++constraint_iter)
  {
    ...
    if(possibly(satisfies(reachable_set,constraint))) {
      ...
      std::vector<timed_set_type> jump_set = this->unforced_jump_step(transition,initial_set,bounding_box,time_step);
  ...
  for(uint i=0; i!=events.size(); ++i) {
    ...
    if(transition.forced()) {
      std::vector<timed_set_type> jump_sets = this->forced_jump_step(transition,initial_set,bounding_box);
      ::append(result,jump_sets);
    } else {
      std::vector<timed_set_type> jump_sets = this->unforced_jump_step(transition,initial_set,bounding_box,time_step);
      ::append(result,jump_sets);
    }
  }
...
Evaluation::ConstraintHybridEvolverPlugin<R>::upper_reachability_step(const System::ConstraintHybridAutomaton<R>& automaton, 
                                                                ...
                                                                time_type& time_step) const
{
  ARIADNE_LOG(2,"HybridEvolverPlugin::upper_reachability_step(...)\n");
