    ...
    : public LocatedSetInterface, public DrawableInterface
{
  ...
  public:
    ...
    ConstrainedImageSet(const BoxSet& dom) : _domain(dom), _function(RealVectorFunction::identity(dom.size())) { }
    //! \brief Construct the image of \a dom under \a fn.
    ConstrainedImageSet(const BoxSet& dom, const RealVectorFunction& fn) : _domain(dom), _function(fn) {
    ...
    ConstrainedImageSet(const BoxSet& dom, const RealVectorFunction& fn, const RealConstraint& c) : _domain(dom), _function(fn), _constraints(1u,c) {
        ARIADNE_ASSERT_MSG(dom.size()==fn.argument_size(),"dom="<<dom<<", fn="<<fn);
    ...
    ConstrainedImageSet(const BoxSet& dom, const RealVectorFunction& fn, const List<RealConstraint>& c) : _domain(dom), _function(fn), _constraints(c) {
        ARIADNE_ASSERT_MSG(dom.size()==fn.argument_size(),"dom="<<dom<<", fn="<<fn); }
    ...
    : public LocatedSetInterface, public DrawableInterface
{
  ...
  public:
    ...
    //! \brief Construct the image of \a dom under \a fn.
    ValidatedConstrainedImageSet(const Vector<Interval>& dom, const IntervalVectorFunction& fn) : _domain(dom), _reduced_domain(dom), _function(fn) {
        ARIADNE_ASSERT_MSG(dom.size()==fn.argument_size(),"dom="<<dom<<", fn="<<fn); }
