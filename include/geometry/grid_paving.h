...
namespace Ariadne {
	...
	template<class R> std::ostream& operator<<(std::ostream& os, const GridPavingCell<R>& gridPavingCell);
	...
	/* PIETER: It might be better to have the GridPavingCell<R> as a data field rather than a subclass. */
	template<class R> class GridPavingCursor {
		private:
			...
			const GridSubPaving<R> * _pSubPaving;
			
			GridPavingCell<R> _theCurrentGridCell;
		...
		public:
			...
			GridPavingCursor(const GridSubPaving<R> * pSubPaving);
			
			/*! \brief The simple copy copnstructor */
			GridPavingCursor(const GridPavingCursor<R> & pSubPaving);
	 ...
	 * The return objects for this iterator are constant GridPavingCell<R>
	 */
	template<class R> class GridPavingConstIterator : public boost::iterator_facade< GridPavingConstIterator<R>, GridPavingCell<R> const, boost::forward_traversal_tag > {
		private:
			...
			GridPavingCursor<R> _pGridPavingCursor;
			 ...
			 */
			bool equal( GridPavingConstIterator<R> const & theOtherIterator) const;
			
			GridPavingCell<R> const& dereference() const;
		...
		public:
			 ...
			 */
			explicit GridPavingConstIterator( const GridSubPaving<R> * pSubPaving, const tribool firstLastNone );
			
			/*! \brief The copy constructor */
			GridPavingConstIterator( const GridPavingConstIterator<R>& gridPavingIter );
			...
			GridPavingCursor<R> const& cursor() const;
	 ...
	 */
	template<class R> class GridPavingCell : public BoxExpression< GridPavingCell<R> > {
		protected:
			 ...
			 */
			GridPavingCell(const Grid<R>& theGrid, const uint theHeight, const BinaryWord& theWord, const Box<R>& theBox);
			
			friend class GridPavingCursor<R>;
		...
		public:
			...
			GridPavingCell(const GridPavingCell<R>& theGridPavingCell);
			 ...
			 */
			GridPavingCell(const Grid<R>& theGrid, const uint theHeight, const BinaryWord& theWord);
			 ...
			 */
			static Box<R> compute_box(const Grid<R>& theGrid, const uint theHeight, const BinaryWord& theWord);
			 ...
			 */
			static Box<R> primary_cell( const uint theHeight, const dimension_type dimensions );
			 ...
			 */
			static uint smallest_primary_cell( const dimension_type dimensions, const Box<R>& theBox );

			/*! \brief Apply grid data \a theGrid to \a theGridBox in order to compute
			 * the box dimensions in the original space
			 */
			static Box<R> grid_box_to_space(const Box<R> & theGridBox, const Grid<R>& theGrid );
	...
	template<class R> class GridSubPaving {
		protected:
			...
			template<class Rl, class Set> friend GridPaving<Rl> outer_approximation( const Grid<Rl>& theGrid, const Set& theSet, const uint depth );
			template<class Rl, class Set> friend GridPaving<Rl> inner_approximation( const Grid<Rl>& theGrid, const Set& theSet, const uint depth );
			...
			GridPavingCell<R> _theGridPavingCell;
			 ...
			 */
			uint compute_number_subdiv( R theWidth, const R theMaxWidth) const;
		
		public:
			/*! \brief A short name for the constant iterator */
			typedef GridPavingConstIterator<R> const_iterator;
			 ...
			 */
			GridSubPaving( const Grid<R>& theGrid, const uint theHeight, const BinaryWord& theWord, BinaryTreeNode * pRootTreeNode );
			...
			*/
			bool operator==(const GridSubPaving<R>& anotherGridSubPaving) const;
			 ...
			 * not subdivided.
			 */
			void subdivide( R theMaxCellWidth );
			...
			operator GridCellListSet<R>() const;
	 ...
	 */
	template<class R> class GridPaving : public GridSubPaving<R> {		
		protected:
			 ...
			 */
			BinaryTreeNode* align_with_cell( const GridPavingCell<R>& theCell, const bool stop_on_enabled, bool & has_stopped );
		...
		public:
			 ...
			 */
			GridPaving( const Grid<R>& theGrid, const uint theHeight, BinaryTreeNode * pRootTreeNode );
			 ...
			 */
			GridPaving( const GridPaving<R> & theGridPaving );
			 ...
			 */
			explicit GridPaving( const Grid<R>& theGrid, const bool enable = false  );

			/*! \brief Construct an empty tree. The \a theBoundingBox is used to define the lattice
			 *  block (in theGrid) that will correspond to the root of the paving tree \a pRootTreeNode.
			 */
			explicit GridPaving( const Grid<R>& theGrid, const Box<R> & theBoundingBox );
			 ...
			 */
			explicit GridPaving( const Grid<R>& theGrid, const IndexArray theLeftLowerPoint,
			 ...
			 */
			explicit GridPaving( const Grid<R>& theGrid, uint theHeight, const BooleanArray& theTree, const BooleanArray& theEnabledCells );
			...
			friend bool subset<>( const GridPavingCell<R>& theCell, const GridSubPaving<R>& theSet );
	
			/*! \brief Tests if a cell is overlaps (intersects as an open set) a paving set. */
			friend bool overlap<>( const GridPavingCell<R>& theCell, const GridSubPaving<R>& theSet );

			/*! \brief Tests if a grid paving set is a subset of another. */
			friend bool subset<>( const GridSubPaving<R>& theSet1, const GridSubPaving<R>& theSet2 );
	
			/*! \brief Tests if two grid paving sets overlap (i.e. intersect as open sets.) */
			friend bool overlap<>( const GridSubPaving<R>& theSet1, const GridSubPaving<R>& theSet2 );
			...
			* approximation. Unfortunately, this is very difficult for subset. */
			...
			friend bool subset<>( const Box<R>& theBox, const GridSubPaving<R>& theSet );
	
			/*! \brief Tests if a box is disjoint from (the closure of) a grid set. */
			friend bool disjoint<>( const Box<R>& theBox, const GridSubPaving<R>& theSet );
	
			/*! \brief Tests if a box intersects thea set. */
			friend bool intersects<>( const Box<R>& theBox, const GridSubPaving<R>& theSet );
			...
			void adjoin( const GridPavingCell<R>& theCell );
			
			/*! \brief Remove a single cell. */
			void remove( const GridPavingCell<R>& theCell );

			/*! \brief Adjoin (make inplace union with) another grid paving set. */
			void adjoin( const GridSubPaving<R>& theOtherSubPaving );
			
			/*! \brief Restrict to (make inplace intersection with) another grid paving set. */
			void restrict( const GridSubPaving<R>& theOtherSubPaving );
			
			/*! \brief Remove cells in another grid paving set. */
			void remove( const GridSubPaving<R>& theOtherSubPaving );

			/*! \brief Join (make union of) two grid paving sets. */
			friend GridPaving<R> join<>( const GridSubPaving<R>& theSet1, const GridSubPaving<R>& theSet2 );
			 ...
			 */
			friend GridPaving<R> intersection<>( const GridSubPaving<R>& theSet1, const GridSubPaving<R>& theSet2 );
	
			/*! \brief The difference of two grid paving sets. */
			friend GridPaving<R> difference<>( const GridSubPaving<R>& theSet1, const GridSubPaving<R>& theSet2 );
...
#include "geometry/grid_paving.inline.h"
