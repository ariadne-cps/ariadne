...
template<class R>
tribool
Geometry::GridTreeSet<R>::intersects(const Rectangle<R>& r) const
{
...
template<class R>
tribool
Geometry::GridTreeSet<R>::disjoint(const Rectangle<R>& r) const
{
  return Geometry::disjoint(*this,r);
}
...
template<class R>
tribool
Geometry::GridTreeSet<R>::superset(const Rectangle<R>& r) const
{
...
template<class R>
tribool
Geometry::GridTreeSet<R>::subset(const Rectangle<R>& r) const
{
...
template<class R>
Geometry::GridTreeSet<R>::operator ListSet< Rectangle<R> >() const 
{
...
template<class R>
tribool
Geometry::disjoint(const GridTreeSet<R>& pts, const Rectangle<R>& r)
{
  tribool result=true;
...
template<class R>
tribool
Geometry::subset(const GridTreeSet<R>& pts, const Rectangle<R>& r)
{
  tribool result=true;
  Rectangle<R> cell(pts.dimension());
  for(typename GridTreeSet<R>::const_iterator pts_iter=pts.begin();
...
template<class R>
tribool
Geometry::subset(const Rectangle<R>& r, const GridTreeSet<R>& pts)
{
...
Geometry::over_approximation(const Rectangle<R>& r, const GridScheme<R>& ps)
{
  if(possibly(r.empty())) {
    ARIADNE_THROW(EmptyInterior,"GridTreeCell<R> over_approximation(const Rectangle<R>& r, const GridScheme<R>& ps)"," with r="<<r);
  }
  
  if(!subset(r,ps.unit_box())) {
    ARIADNE_THROW(std::runtime_error,"ParttitionTreeCell over_approximation(Rectangle r, GridScheme ps, uint d)"," with r="<<r<<", ps="<<ps<<": r is not a subset of ps.unit_box()");
  }
  ...
  while(true) {
    ...
    if(!subset(r,Rectangle<R>(GridTreeCell<R>(unit_box,subdivisions,new_word)))) {
      new_word.pop_back();
      word.push_back(Combinatoric::BinaryTree::right);
      if(!subset(r,Rectangle<R>(GridTreeCell<R>(unit_box,subdivisions,word)))) {
        break;
      }
    }
  }
...
Geometry::outer_approximation(const S& s, const Grid<R>& g, const uint depth)
{
  ...
  do {
    Rectangle<R> cell=Rectangle<R>(GridTreeCell<R>(bounding_box,subdivisions,word));
...
Geometry::inner_approximation(const S& s, const GridScheme<R>& ps, const uint depth)
{
  ...
  do {
    Rectangle<R> cell=Rectangle<R>(GridTreeCell<R>(unit_box,subdivisions,word));
