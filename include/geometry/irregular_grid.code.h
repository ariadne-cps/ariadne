...
namespace Ariadne {
  namespace Geometry {
    ...
    template<class R> 
    index_type 
    IrregularGrid<R>::subdivision_index(dimension_type d, const real_type& x) const 
    {
      const array<R>& subdiv=this->_subdivision_coordinates[d];
    ...
    template<class R> 
    index_type 
    IrregularGrid<R>::subdivision_lower_index(dimension_type d, const real_type& x) const 
    {
      const array<R>& subdiv=this->_subdivision_coordinates[d];
      if(x < subdiv.front() || x > subdiv.back()) {
        std::cerr << __FUNCTION__ << *this << " " << d << "  " << x << std::endl;
    ...
    template<class R> 
    index_type 
    IrregularGrid<R>::subdivision_upper_index(dimension_type d, const real_type& x) const 
    {
      const array<R>& subdiv=this->_subdivision_coordinates[d];
      if(x < subdiv.front() || x > subdiv.back()) {
        std::cerr << __FUNCTION__ << *this << " " << d << "  " << x << std::endl;
    ...
    IrregularGrid<R>::index_block(const Rectangle<R>& r) const 
    { 
    ...
    template<class R> inline 
    Rectangle<R>
    IrregularGrid<R>::rectangle(const Combinatoric::LatticeBlock& lb) const 
    { 
