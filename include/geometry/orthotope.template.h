...
namespace Ariadne {
  namespace Geometry {
    ...
    Orthotope<RC,RG>::Orthotope(const Point<R0>& c, 
                              const LinearAlgebra::Matrix<R1>& g)
      : _centre(c),
        _generators(g)
    {
      if(c.dimension()!=g.number_of_rows()) { 
        ARIADNE_THROW(InvalidGenerators,"Orthotope::Orthotope(Point c,Matrix g)","c="<<c<<", g="<<g);
      }
    }
    ...
    { 
      if(c.dimension()!=g1.number_of_rows() || c.dimension()!=g2.size()) { 
        ARIADNE_THROW(InvalidGenerators,"Orthotope::Orthotope(Point c, Matrix g1, Vector g2)","c="<<c<<", g1="<<g1<<", g2="<<g2);
      }
    ...
    { 
      if(c.dimension()!=g1.number_of_rows() || c.dimension()!=g2.number_of_rows()) { 
        ARIADNE_THROW(InvalidGenerators,"Orthotope::Orthotope(Point c, Matrix g1, Matrix g2)","c="<<c<<", g1="<<g1<<", g2="<<g2);
      }
    }
