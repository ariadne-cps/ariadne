...
class DiscreteMode {
  ...
  public:
    ...
    boost::shared_ptr<const FunctionInterface> dynamic() const {
...
class DiscreteTransition
{
  ...
  public:
    ...
    //! \brief The activation region of the discrete transition.
    boost::shared_ptr<const FunctionInterface> activation() const {
        return this->_activation;
    }

    //! \brief The reset map of the discrete transition.
    boost::shared_ptr<const FunctionInterface> reset() const {
        return this->_reset;
    }
  ...
  private:
    // Construct from shared pointers (for internal use only).
 ...
 *
 * A discrete transision can either be \em urgent or \em unurgent.
 * A urgent transition much occur as soon as it is activated.
 * An unurgent transition may occur at any time it is activated,
 * but is only urgent to occur if the continuous evolution is
