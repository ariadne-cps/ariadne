...
class PolyhedronSetExporter {
 ...
 public:
  ...
  PolyhedronSetExporter(const std::string &file_name, const Rectangle &bbox)
    : _e(file_name,bbox) {}
  ...
  PolyhedronSetExporter(const std::string &file_name, std::vector<uint> dims)
    : _e(file_name, dims) {}
...
class PolyhedronOneDimMatlabExporter {
 ...
 public:
  PolyhedronOneDimMatlabExporter() 
    : _os_given(false), _with_dims(false), _data_line(0) {}
  PolyhedronOneDimMatlabExporter(const std::string& file_name, const Rectangle& bbox)
    : _os_given(false), _f_name(file_name), _with_dims(false), _data_line(0) {}
  PolyhedronOneDimMatlabExporter(const std::string& file_name, const Rectangle& bbox, const std::vector<uint>& dims)
    : _os_given(false), _f_name(file_name), _with_dims(true), _dims(dims), _data_line(0) 
  {
    if (dims.size()!=1) {
      throw "The PolyhedronOneDimMatlabExporter export on one dimension only.";
    }				
  }
  PolyhedronOneDimMatlabExporter(const std::string& file_name, const std::vector<uint>& dims)
    : _os_given(false), _f_name(file_name), _with_dims(true), _dims(dims), _data_line(0) 
  {
    if (dims.size()!=1) {
      throw "The PolyhedronOneDimMatlabExporter export on one dimension only.";
  ...
  PolyhedronOneDimMatlabExporter(std::ofstream os, const Rectangle& bbox)
    : _os_given(true), _os(os), _with_dims(false), _data_line(0) {}
  PolyhedronOneDimMatlabExporter(std::ofstream os, const Rectangle& bbox, const std::vector<uint>& dims)
    : _os_given(true), _os(os), _with_dims(true), _dims(dims),_data_line(0)
  {
    if (dims.size()!=1) {
      throw "The PolyhedronOneDimMatlabExporter export on one dimension only.";
    }				
  ...
  void export_basicset(const BasicSet& p, const std::vector<uint>& dims) {
    this->_open_stream_if_not_opened();
