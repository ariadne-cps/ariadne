    ...
    : public Loggable
{
  public:
    ...
    virtual Vector<ValidatedNumberType> zero(const ValidatedVectorFunction& f,const ExactBox& pt) const = 0;
    //! \brief Solve \f$f(x)=x\f$, starting in the box \a bx.
    virtual Vector<ValidatedNumberType> fixed_point(const ValidatedVectorFunction& f,const ExactBox& pt) const = 0;

    //! \brief Solve \f$f(x)=0\f$, starting in the box \a bx. Throws a SolverException if there is not a unique solution.
    virtual Vector<ValidatedNumberType> solve(const ValidatedVectorFunction& f,const ExactBox& bx) const = 0;
    ...
    virtual ValidatedVectorFunctionModel continuation(const ValidatedVectorFunction& f, const Vector<ApproximateNumberType>& a, const ExactBox& X,  const ExactBox& A) const = 0;
    ...
    //!
    virtual Set< Vector<ValidatedNumberType> > solve_all(const ValidatedVectorFunction& f,const ExactBox& bx) const = 0;
};
