...
=======
  ...
  // Compute the norm_2 on a vector
  template<class X> X
  norm2(const Vector<X> &A);

  // Compute the norm_1 on a vector
  template<class X> X
  norm1(const Vector<X> &A);

  // Jacobi computation of eigenvalues
  template<class X> Tuple< Matrix<X>, Vector<X> >
  jacobi_eigs(const Matrix<X> &A);

  // Power method computation of eigenvalues
  template<class X> Tuple<Vector<X>, X>
  power_eigs(const Matrix<X> &A,const unsigned itmax=100);

  // Inverse power method computation of eigenvalues:
  //    compute power method with (A-muI)^-1
  //    taken from https://en.wikiversity.org/wiki/Shifted_inverse_iteration
  template<class X> Tuple<Vector<X>, X>
  inverse_power_eigs(const Matrix<X> &A, const X &mu, const unsigned itmax = 100);
