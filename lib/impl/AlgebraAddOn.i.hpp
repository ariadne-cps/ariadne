...
lp_min(const Vector<X> &C, const Matrix<X> &A, const Vector<X> &b, const Vector<X> &lb, int &errnum)
{
  // constants
  const int       n=C.size();
  const int       m=b.size();
  ...
  for(int i=0;i<m;++i)
  {
    for(int j=0;j<n;++j)
    {
      ia[cu(index)]=i+1,ja[cu(index)]=j+1,ar[cu(index)]=cd(A[cu(i)][cu(j)].dbl);
...
=======
  ...
  for(int i=0;i<n;++i)
  {
    if(is_inf(lb[cu(i)]))
    {
      glp_set_col_bnds(lp,i+1,GLP_FR,-std::numeric_limits<double>::infinity(),std::numeric_limits<double>::infinity());
    }
    else
    {
      glp_set_col_bnds(lp, i+1, GLP_LO,cd(lb[cu(i)].dbl),std::numeric_limits<double>::infinity());
    }
    glp_set_obj_coef(lp,i+1,cd(C[cu(i)].dbl));
  }
...
Tuple<Matrix<X>, Vector<X>> jacobi_eigs(const Matrix<X> &matrix) {
  ...
  for (i = 0; i < n; i++) {
=======
template<class X> Tuple< Matrix<X>, Vector<X> >
jacobi_eigs (const Matrix<X> &matrix)
{
  Matrix<X>   A = matrix;
  unsigned    n = A.row_size();
  unsigned    it_max = 100;
...
power_eigs(const Matrix<X> &matrix, const unsigned itmax)
{
  const unsigned  n = matrix.row_size();
  Matrix<X>       A = matrix;
  Vector<X>       lambda = Vector<X>::one(n);
...
template<class X> Tuple<Vector<X>, X>
inverse_power_eigs(const Matrix<X> &matrix, const X &mu, const unsigned itmax)
{
  const X         rtol = static_cast<X>(std::numeric_limits<double>::epsilon());
  const unsigned  n = matrix.row_size();
  Matrix<X>       A;
...
=======
  if(is_null) //<! Matrix is null, then e_1 is the smallest and 0 is the eigenvalue
  {
    Vector<X> null_ret = Vector<X>::zero(n);
    null_ret[0]=static_cast<X>(1.0);
    return make_tuple(null_ret, static_cast<X>(0));
...
eigen_eigs(const Matrix<X> &A)
{
  ...
  Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> solver(eigenA,Eigen::ComputeEigenvectors);
  auto eigenValues = solver.eigenvalues();
  auto eigenVectors = solver.eigenvectors();
  unsigned index = 0;
  for(unsigned i = 0; i<eigenValues.size();++i)
