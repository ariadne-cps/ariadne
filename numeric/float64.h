...
template<class P> struct IsScalar<Float64<P>> : True { };
...
struct DispatchFloat64Arithmetic {
    ...
    template<class P, class X, EnableIf<IsNonFloatNumber<X>> =dummy> friend auto operator+(Float<P> f, X x) -> decltype(add_number(f,x)) { return add_number(f,x); }
    template<class P, class X, EnableIf<IsNonFloatNumber<X>> =dummy> friend auto operator+(X x, Float<P> f) -> decltype(add_number(x,f)) { return add_number(x,f); }

    template<class P, class X, EnableIf<IsNonFloatNumber<X>> =dummy> friend auto operator-(Float<P> f, X x) -> decltype(sub_number(f,x)) { return sub_number(f,x); }
    template<class P, class X, EnableIf<IsNonFloatNumber<X>> =dummy> friend auto operator-(X x, Float<P> f) -> decltype(sub_number(x,f)) { return sub_number(x,f); }
    template<class P, class X, EnableIf<IsNonFloatNumber<X>> =dummy> friend auto operator*(Float<P> f, X x) -> decltype(mul_number(f,x)) { return mul_number(f,x); }
    template<class P, class X, EnableIf<IsNonFloatNumber<X>> =dummy> friend auto operator*(X x, Float<P> f) -> decltype(mul_number(x,f)) { return mul_number(x,f); }
    template<class P, class X, EnableIf<IsNonFloatNumber<X>> =dummy> friend auto operator/(Float<P> f, X x) -> decltype(div_number(f,x)) { return div_number(f,x); }
    template<class P, class X, EnableIf<IsNonFloatNumber<X>> =dummy> friend auto operator/(X x, Float<P> f) -> decltype(div_number(x,f)) { return div_number(x,f); }

/*
    // Mixed arithmetic with generic number types
    template<class P1, class P2> friend auto operator+(Float<P1> f, Number<P2> n) -> decltype(add_number(f,n)) { return add_number(f,n); }
    template<class P1, class P2> friend auto operator+(Number<P1> n, Float<P2> f) -> decltype(add_number(n,f)) { return add_number(n,f); }

    template<class P1, class P2> friend auto operator-(Float<P1> f, Number<P2> n) -> decltype(sub_number(f,n)) { return sub_number(f,n); }
    template<class P1, class P2> friend auto operator-(Number<P1> n, Float<P2> f) -> decltype(sub_number(n,f)) { return sub_number(n,f); }

    template<class P1, class P2> friend auto operator*(Float<P1> f, Number<P2> n) -> decltype(mul_number(f,n)) { return mul_number(f,n); }
    template<class P1, class P2> friend auto operator*(Number<P1> n, Float<P2> f) -> decltype(mul_number(n,f)) { return mul_number(n,f); }

    template<class P1, class P2> friend auto operator/(Float<P1> f, Number<P2> n) -> decltype(div_number(f,n)) { return div_number(f,n); }
    template<class P1, class P2> friend auto operator/(Number<P1> n, Float<P2> f) -> decltype(div_number(n,f)) { return div_number(n,f); }

    // Mixed arithmetic with concrete real number types
    template<class P> friend auto operator+(Float<P> f, Real n) -> decltype(add_number(f,n)) { return add_number(f,n); }
    template<class P> friend auto operator+(Real n, Float<P> f) -> decltype(add_number(n,f)) { return add_number(n,f); }

    template<class P> friend auto operator-(Float<P> f, Real n) -> decltype(sub_number(f,n)) { return sub_number(f,n); }
    template<class P> friend auto operator-(Real n, Float<P> f) -> decltype(sub_number(n,f)) { return sub_number(n,f); }

    template<class P> friend auto operator*(Float<P> f, Real n) -> decltype(mul_number(f,n)) { return mul_number(f,n); }
    template<class P> friend auto operator*(Real n, Float<P> f) -> decltype(mul_number(n,f)) { return mul_number(n,f); }

    template<class P> friend auto operator/(Float<P> f, Real n) -> decltype(div_number(f,n)) { return div_number(f,n); }
    template<class P> friend auto operator/(Real n, Float<P> f) -> decltype(div_number(n,f)) { return div_number(n,f); }
*/
    // Mixed arithmetic with builtin floating-point type
    using ApF = Float<Approximate>;
    template<class P, class D, EnableIf<IsFloatingPoint<D>> =dummy> friend auto operator+(Float<P> f, D d) -> decltype(add(f,ApF(d))) { return add(f,ApF(d)); }
    template<class P, class D, EnableIf<IsFloatingPoint<D>> =dummy> friend auto operator+(D d, Float<P> f) -> decltype(add(ApF(d),f)) { return add(ApF(d),f); }

    template<class P, class D, EnableIf<IsFloatingPoint<D>> =dummy> friend auto operator-(Float<P> f, D d) -> decltype(sub(f,ApF(d))) { return sub(f,ApF(d)); }
    template<class P, class D, EnableIf<IsFloatingPoint<D>> =dummy> friend auto operator-(D d, Float<P> f) -> decltype(sub(ApF(d),f)) { return sub(ApF(d),f); }

    template<class P, class D, EnableIf<IsFloatingPoint<D>> =dummy> friend auto operator*(Float<P> f, D d) -> decltype(mul(f,ApF(d))) { return mul(f,ApF(d)); }
    template<class P, class D, EnableIf<IsFloatingPoint<D>> =dummy> friend auto operator*(D d, Float<P> f) -> decltype(mul(ApF(d),f)) { return mul(ApF(d),f); }

    template<class P, class D, EnableIf<IsFloatingPoint<D>> =dummy> friend auto operator/(Float<P> f, D d) -> decltype(div(f,ApF(d))) { return div(f,ApF(d)); }
    template<class P, class D, EnableIf<IsFloatingPoint<D>> =dummy> friend auto operator/(D d, Float<P> f) -> decltype(div(ApF(d),f)) { return div(ApF(d),f); }
...
template<class X, class D, EnableIf<And<IsNumber<X>,IsFloatingPoint<D>>> =dummy> inline auto
    operator==(X x, D d) -> decltype(declval<X>()==declval<ApproximateFloat64>()) { return x==Float64<Approximate>(d); }
template<class X, class D, EnableIf<And<IsNumber<X>,IsFloatingPoint<D>>> =dummy> inline auto
    operator!=(X x, D d) -> decltype(declval<X>()!=declval<ApproximateFloat64>()) { return x!=Float64<Approximate>(d); }
template<class X, class D, EnableIf<And<IsNumber<X>,IsFloatingPoint<D>>> =dummy> inline auto
    operator< (X x, D d) -> decltype(declval<X>()< declval<ApproximateFloat64>()) { return x< Float64<Approximate>(d); }
...
template<> class Float64<Exact>
    : DispatchFloat64Arithmetic
{
    friend class Float64<Bounded>;
    friend class Float64<Approximate>;
    volatile double _v;
  ...
  public:
    ...
    Float64<Exact>();
    template<class N, EnableIf<IsIntegral<N>> =dummy> Float64<Exact>(N);
    explicit Float64<Exact>(Integer);
    explicit Float64<Exact>(Flt64);
...
Float64<Exact>::Float64(N n) : Float64<Exact>(Int64(n)) { assert(this->_v==n); }
...
template<> class Float64<Error> {
  ...
  public:
    ...
    Float64<Error>();
    template<class M, EnableIf<IsSame<M,uint>> = dummy> Float64<Error>(M m);
    template<class X, EnableIf<IsSame<X,double>> = dummy> explicit Float64<Error>(X x);
    explicit Float64<Error>(double);
    explicit Float64<Error>(Flt64);
    explicit Float64<Error>(UpperFloat64);
    operator UpperFloat64 () const;
    Flt64 get_flt() const;
    double get_d() const;
    ...
    : DispatchFloat64Arithmetic
{
  ...
  public:
    static void set_output_precision(int);
  private:
    Float64<Metric>(double d, ExactTag);
  public:
    typedef Validated Paradigm;
    typedef MetricFloat64 NumericType;

    Float64<Metric>();
    explicit Float64<Metric>(double);
    explicit Float64<Metric>(double,double);
    explicit Float64<Metric>(Flt64);
    Float64<Metric>(ExactFloat64, ErrorFloat64);
    Float64<Metric>(LowerFloat64, UpperFloat64) = delete;

    template<class N, EnableIf<IsIntegral<N>> =dummy> Float64<Metric>(N n);
    explicit Float64<Metric>(Integer const&);
    explicit Float64<Metric>(Rational const&);
    explicit Float64<Metric>(Real const&);
...
template<class N, EnableIf<IsIntegral<N>>> inline
Float64<Metric>::Float64(N n) : Float64<Metric>(double(n),ExactTag()) { }
...
template<> class Float64<Bounded>
    : DispatchFloat64Arithmetic
...
{
    friend class Float64<Approximate>; friend class Float64<Metric>;
  ...
  public:
    ...
    Float64<Bounded>();
    explicit Float64<Bounded>(double);
    explicit Float64<Bounded>(double,double);
    explicit Float64<Bounded>(Flt64);
    Float64<Bounded>(LowerFloat64, UpperFloat64);
    Float64<Bounded>(LowerFloat64, ErrorFloat64) = delete;

    template<class N, EnableIf<IsIntegral<N>> =dummy>
        Float64<Bounded>(N n) : Float64<Bounded>(double(n),Exact()) { }
    template<class X, EnableIf<IsFloatingPoint<X>> =dummy>
        explicit Float64<Bounded>(X x) : Float64<Bounded>(double(x),Exact()) { }

    explicit Float64<Bounded>(Integer const&);
    explicit Float64<Bounded>(Rational const&);
    explicit Float64<Bounded>(Real const&);
    ...
    : DispatchFloat64Arithmetic
{
  ...
  public:
    ...
    template<class N, EnableIf<IsIntegral<N>> =dummy>
        Float64<Upper>(N n) : Float64<Upper>(double(n),Exact()) { }
    template<class X, EnableIf<IsFloatingPoint<X>> =dummy>
        explicit Float64<Upper>(X x) : Float64<Upper>(double(x),Exact()) { }

    explicit Float64<Upper>(Flt64);
    explicit Float64<Upper>(Integer const&);
    explicit Float64<Upper>(Rational const&);
    explicit Float64<Upper>(Real const&);
    ...
    : DispatchFloat64Arithmetic
{
  ...
  public:
    ...
    template<class N, EnableIf<IsIntegral<N>> =dummy>
        Float64<Lower>(N n) : Float64<Lower>(double(n),Exact()) { }
    template<class X, EnableIf<IsFloatingPoint<X>> =dummy>
        explicit Float64<Lower>(X x) : Float64<Lower>(double(x),Exact()) { }
    explicit Float64<Lower>(Flt64);
    explicit Float64<Lower>(Integer const&);
    explicit Float64<Lower>(Rational const&);
    explicit Float64<Lower>(Real const&);
...
template<> class Float64<Approximate>
    : DispatchFloat64Arithmetic
{
    static int output_precision;
    volatile double _a;
  ...
  public:
    ...
    Float64<Approximate>();
    Float64<Approximate>(double);
    Float64<Approximate>(Flt64);
    explicit Float64<Approximate>(Integer const&);
    explicit Float64<Approximate>(Rational const&);
    explicit Float64<Approximate>(Real const&);
...
class TwoExp {
  ...
  public:
    ...
    operator ExactFloat64 () const { return Float64<Exact>(this->get_d()); }
    operator ErrorFloat64 () const { return Float64<Error>(this->get_d()); }
    operator MetricFloat64 () const { return Float64<Metric>(this->get_d()); }
    operator BoundedFloat64 () const { return Float64<Bounded>(this->get_d()); }
};
