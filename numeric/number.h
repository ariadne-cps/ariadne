...
template<class P> class Number
{
  ...
  public:
   ...
   // template<class PP, DisableIfWeaker<P,PP> =dummy> Number(const Number<PP>& y) = delete;
    ...
    template<class X, EnableIf<And<IsSame<P,Approximate>,IsFloatingPoint<X>>> =dummy> Number(const X& x) : Number<P>(Float64<P>(x)) { }
    ...
    template<class PP, EnableIf<And<IsWeaker<PP,P>,Not<IsWeaker<Effective,PP>>>> = dummy>
        explicit operator Float64<PP>() const { return this->get(PP()); }

    //! \brief Get the value of the number as a double-precision type
    template<class PP> Float64<PP> get(PP par, Precision64 const& prec) const { return pointer()->_get(PP(),prec); }
    //! \brief Get the value of the number as a multiple-precision type
    template<class PP> FloatMP<PP> get(PP par, PrecisionMP const& prec) const { return pointer()->_get(PP(),prec); }
    ...
    template<class PP, EnableIf<IsWeaker<PP,P>> = dummy>
        Float64<PP> get(PP p) const { return this->ref()._get(p); }
    template<class PP, EnableIf<IsWeaker<PP,P>> = dummy>
        Float64<PP> get(PP p, Precision64 const& pr) const { return this->ref()._get(p); }

    //! \brief Get the value of the number represented by \a X with the same precision paramters as \a p.
    template<class PP, EnableIf<IsWeaker<PP,P>> = dummy>
        FloatMP<PP> get(PP p, PrecisionMP const& pr) const { return this->ref()._get(p,pr); }
