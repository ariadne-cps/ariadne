...
template<> struct RealConstant<Integer> : RealInterface, BoundFloat {
  ...
  public:
    RealConstant(X const& x) : BoundFloat(x), _c(x) { }
    virtual BoundFloat _value() const { return static_cast<BoundFloat const&>(*this); }
    virtual BoundFloatMP _evaluate(PrecisionMP pr) const { return BoundFloatMP(_c,pr); }
    virtual OutputStream& _write(OutputStream& os) const { return os << this->_c; }
};
...
Float64<Metrc>::Float64(Real const& r) : Float64<Metrc>(r._ptr->_value()) { }
Float64<Bound>::Float64(Real const& r) : Float64<Bound>(r._ptr->_value()) { }
Float64<Upper>::Float64(Real const& r) : Float64<Upper>(r._ptr->_value()) { }
Float64<Lower>::Float64(Real const& r) : Float64<Lower>(r._ptr->_value()) { }
Float64<Apprx>::Float64(Real const& r) : Float64<Apprx>(r._ptr->_value()) { }

FloatMP<Metrc>::FloatMP(Real const& r, PrecisionMP pr) : FloatMP<Metrc>(r._ptr->_evaluate(pr)) { }
FloatMP<Bound>::FloatMP(Real const& r, PrecisionMP pr) : FloatMP<Bound>(r._ptr->_evaluate(pr)) { }
FloatMP<Upper>::FloatMP(Real const& r, PrecisionMP pr) : FloatMP<Upper>(r._ptr->_evaluate(pr)) { }
FloatMP<Lower>::FloatMP(Real const& r, PrecisionMP pr) : FloatMP<Lower>(r._ptr->_evaluate(pr)) { }
FloatMP<Apprx>::FloatMP(Real const& r, PrecisionMP pr) : FloatMP<Apprx>(r._ptr->_evaluate(pr)) { }

UpperFloat Real::upper() const { return this->_ptr->_value(); }
LowerFloat Real::lower() const { return this->_ptr->_value(); }
ApprxFloat Real::approx() const { return this->_ptr->_value(); }
...
NegSierpinski operator==(Real x1, Int64 n2) { return BoundFloat(x1)==n2; }
Sierpinski operator!=(Real x1, Int64 n2) { return BoundFloat(x1)!=n2; }
Tribool operator< (Real x1, Int64 n2) { return BoundFloat(x1)< n2; }
Tribool operator> (Real x1, Int64 n2) { return BoundFloat(x1)> n2; }
Tribool operator<=(Real x1, Int64 n2) { return BoundFloat(x1)<=n2; }
Tribool operator>=(Real x1, Int64 n2) { return BoundFloat(x1)>=n2; }
