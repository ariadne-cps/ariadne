...
# Location fly:
dyn=AffineVectorField(Matrix("[0, 1; 0, 0]"), Vector("[0,-9.8]"))
inv=PolyhedralSet(Rectangle("[0,15]x[-20,20]"))
# inv=PolyhedralSet(Polyhedron(Matrix("[-1,0]"),Vector("[0]")))
l1=automaton.new_mode(0, dyn, inv)

# Transition l1 -> l1
#act=Polyhedron(Matrix("[1,0; 0,1]"),Vector("[0,0]"))
act=Polyhedron(Rectangle("[0,0.1]x[-20,20]"))
res=AffineMap(Matrix("[1,0;0,-1]"), Vector("[0,0]"))
...
block=LatticeBlock("[-10,100]x[-100,100]")
fgrid=FiniteGrid(grid,block)

# Initial set 
init=Rectangle("[10,10.1]x[0,0.1]")
...
# Bounding set 
bounding_box=Rectangle("[-1,16]x[-21,21]")
