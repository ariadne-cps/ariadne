...
template<class TUP, std::size_t N> inline void write_tuple(std::ostream& os, TUP const& tup, std::integral_constant<std::size_t,N>) {
    if constexpr (N!=1) { write_tuple(os,tup,std::integral_constant<std::size_t,N-1>()); os << ','; } os << std::get<N-1>(tup); }
template<class... TS> OutputStream& operator<<(OutputStream& os, Tuple<TS...> const& tup) {
...
template<> struct ExpressionVariantTrait<Real> {
    typedef Variant<   Constant<Real>
                    ,  Variable<Real>
                    ,  Symbolic<OperatorVariant<Neg,Rec,Exp>,Expression<Real>>
...
template<> struct ExpressionVariantTrait<Vector<Real>> {
    typedef Variant< Constant<Vector<Real>>,
                     ...
                     Symbolic<OperatorVariant<Neg>,Expression<Vector<Real>>>,
...
template<> class Expression<Real> : public ExpressionOperations<Real> {
  ...
  public:
    ...
    Expression(Constant<T> c);
    Expression(Variable<T> v);
    Expression(OperatorVariant<Neg,Rec,Exp>, Expression<T> const&);
...
Expression<Real>::Expression(OperatorVariant<Neg,Rec,Exp> op, Expression<T> const& e)
    :  _node(std::make_shared<ExpressionNode<T>>(Symbolic<OperatorVariant<Neg,Rec,Exp>,Expression<T>>(op,e))) { }
...
template<> class Expression<Vector<Real>> : public ExpressionOperations<Vector<Real>> {
  ...
  public:
    ...
    Expression(OperatorVariant<Neg>, Expression<T>);
...
Expression<Vector<Real>>::Expression(OperatorVariant<Neg> op, Expression<T> e)
    :  _node(std::make_shared<ExpressionNode<T>>(Symbolic<OperatorVariant<Neg>,Expression<T>>(op,e))) { }
...
template<> auto Variable<Vector<Real>>::operator[] (SizeType i) const -> Expression<Real> {
    return get(Expression<Vector<Real>>(*this),i); }
...
template<class T> struct ExpressionWriter {
...
template<class T> OutputStream& ExpressionNode<T>::_write(OutputStream& os) const {
    std::visit([&os](auto en){ExpressionWriter<T>::_write(os,en);},*this); return os; }

OutputStream& operator<<(OutputStream& os, Expression<Real> const& e) {
    return ExpressionWriter<Real>::write(os,e); }
OutputStream& operator<<(OutputStream& os, Expression<Vector<Real>> const& e) {
    return ExpressionWriter<Vector<Real>>::write(os,e); }
...
template<class... AS> class Space {
  ...
  public:
    ...
    friend OutputStream& operator<<(OutputStream& os, Space<AS...> const& spc) { return os << spc._vars; }
};
...
class ExpressionDomain {
  ...
  public:
    ...
    friend OutputStream& operator<<(OutputStream& os, ExpressionDomain const& edom) {
        return os << "ExpressionDomain(" << edom._ivls << "," << edom._bxs << ")"; }
  public:
...
template<class R> class ExpressionPatch : public Expression<R> {
  ...
  public:
    ExpressionPatch(ExpressionDomain edom, Expression<R> const& e) : Expression<R>(e), _dom(edom) { }
    using Expression<R>::Expression;
...
template<class R> struct ConstantOrVariable : public Variant<Constant<R>,Variable<R>> {
    using Variant<Constant<R>,Variable<R>>::variant;
...
//    typedef decltype(domain(declval<AS>()...)) DomainType;
  ...
  public:
    ...
    ExpressionPatch<R> operator() (ConstantOrVariable<AS>... as) const {
        ...
        auto astup=std::make_tuple(as...);
        std::cerr<<"  astup="<<astup<<"\n";
        auto d0=std::get<0>(_d); auto cv0=std::get<0>(astup);
        std::cerr<<"  d0="<<d0<<", cv0="<<cv0<<"\n";
        if constexpr (Same<decltype(cv0),ConstantOrVariable<Real>>) {
            if (std::holds_alternative<RealVariable>(cv0)) { edom[std::get<RealVariable>(cv0)]=d0; } }
        if constexpr (Same<decltype(cv0),ConstantOrVariable<RealVector>>) {
            if (std::holds_alternative<RealVectorVariable>(cv0)) { edom[std::get<RealVectorVariable>(cv0)]=d0; } }
        auto d1=std::get<1>(_d); auto cv1=std::get<1>(astup);
        std::cerr<<"  d1="<<d1<<", cv1="<<cv1<<"\n";
        if constexpr (Same<decltype(cv1),ConstantOrVariable<Real>>) {
            if (std::holds_alternative<RealVariable>(cv1)) { edom[std::get<RealVariable>(cv1)]=d1; } }
        if constexpr (Same<decltype(cv1),ConstantOrVariable<RealVector>>) {
            if (std::holds_alternative<RealVectorVariable>(cv1)) { edom[std::get<RealVectorVariable>(cv1)]=d1; } }
        std::cerr <<"  edom="<<edom<<"\n";
        //apply([&][auto cv]{if constexpr Same<decltype(cv),RealVariable>{edom[cv]=
...
//    ContinuousFunctionPatch<Real(RealVector,Real)>::DomainType dom(bx,ivl);
    ...
    ContinuousFunction<Real(RealVector,Real)> f=ExpressionFunction<Real(RealVector,Real)>({x,t},x[0]*exp(t)+x[1]);
    ContinuousFunction<Real(Real,RealVector)> fr({t,x}, f(xe,te));

    using SIG=Real(RealVector,Real);
    ContinuousFunctionPatch<SIG> fp(dom,f);

    std::cout << "fp=" << fp << std::endl;

    std::cout << "fp(x0,t0)=" << fp(x0,t0) << std::endl;
    std::cout << "fp(x,t)=" << fp(x,t) << std::endl;

    ContinuousFunctionPatch<Real(Real,RealVector)> fpr({t,x}, fp(x,t));
