    ...
    : public Loggable
{
  public:
    ...
    virtual ValidatedVectorFunctionModel implicit(const ValidatedVectorFunction& f, const ExactBoxType& A, const ExactBoxType& X) const = 0;
    //! \brief Solve \f$f(a,x)=0\f$ for a in \a A, looking for a solution with x in \a X.
    virtual ValidatedScalarFunctionModel implicit(const ValidatedScalarFunction& f, const ExactBoxType& A, const ExactIntervalType& X) const = 0;

    //! \brief Solve \f$f(a,x)=0\f$ for x in \a X, and continue to a function \f$h\f$ solving \f$f(a,h(a))=0\f$ over \f$A\f$.
    virtual ValidatedVectorFunctionModel continuation(const ValidatedVectorFunction& f, const Vector<ApproximateNumericType>& a, const ExactBoxType& X,  const ExactBoxType& A) const = 0;
