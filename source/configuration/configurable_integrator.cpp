/***************************************************************************
 *            configuration/configurable_integrator.cpp
 *
...
#include "configurable_integrator.hpp"

namespace Ariadne {


template<> const char* attribute_name<MaximumTemporalOrder> = "maximum_temporal_order";

void test() {
    typedef TaylorPicardIntegrator I;

    I default_integrator(maximum_error=1.0);

    Tuner<I> tuner(default_integrator);
    tuner.new_property<ApproximateDouble>("maximum_error", &I::set_maximum_error, LogSpace(1.0,4.0,3))
//         .new_property<ApproximateDouble>("lipschitz_tolerance", &I::set_lipschitz_tolerance)
         .new_attribute<LipschitzTolerance>()
         .new_property<Nat>("minimum_temporal_order", &I::set_minimum_temporal_order, RangeSpace(4,10))
//         .new_property<Nat>("maximum_temporal_order", &I::set_maximum_temporal_order);
         .new_attribute<MaximumTemporalOrder>(RangeSpace(10,12))
         .new_class<BounderHandle>("Bounder",EulerBounder());
    ;
    tuner.set_property_values<ApproximateDouble>("maximum_error", LogSpace(0.5,4.0,4));
    tuner.set_attribute_values<LipschitzTolerance>(LogSpace(0.5,4.0,4));
}
...
//FIXME
    : IntegratorBase(Configuration<TaylorPicardIntegrator>()), _sweeper(sweeper), _order(ord)
{ }
...
//FIXME
    : IntegratorBase(Configuration<TaylorPicardIntegrator>()), _step_maximum_error(cast_exact(stperr.value())), _sweeper(sweeper)
...
//FIXME
    : IntegratorBase(Configuration<TaylorPicardIntegrator>()) { }
