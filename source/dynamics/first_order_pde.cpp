...
first_order_pde(Matrix<Real> rA, Array<Matrix<Real>> rBs, Array<DiagonalMatrix<Real>> const& rDs, Array<Matrix<Real>> const& rTs, EffectiveVectorMultivariateFunction const& f, EffectiveVectorMultivariateFunction const& phi0, DP pr) {
    ...
    Array<Value<X>> xs(two_pow_N+1, [N,pr](SizeType is){return Value<X>(Dyadic(is,N),pr);});
    // Generate array of x values (1,3,...,2*2^N-1)/2^(N+1)
    Array<Value<X>> hxs(two_pow_N, [N,pr](SizeType is){return Value<X>(Dyadic(2*is+1,N+1),pr);});
    ...
    assert(rTs.size()==2);
    Array<Matrix<Bounds<X>>> Ts(rTs.size(), [&](SizeType is){return Matrix<Bounds<X>>(rTs[is],pr);});
    Array<Matrix<Bounds<X>>> invTs(Ts.size(), [&](SizeType is){return inverse(Ts[is]);});

    Array<DiagonalMatrix<Bounds<X>>> Ds(rDs.size(), [&](SizeType is){return DiagonalMatrix<Bounds<X>>(rDs[is],pr);});
