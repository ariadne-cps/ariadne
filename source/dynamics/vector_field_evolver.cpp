                              ...
                              SharedPointer<SynchronisedOrbit> result) const
{
    ...
    LOGGING_PRINTLN_AT(1,"working_timed_set_model = "<<working_timed_set_model)
    ...
    if (this->_configuration->enable_reconditioning() && possibly(norm(current_set.state_function().errors()) > this->_configuration->maximum_spacial_error())) {
        LOGGING_PRINTLN("reconditioning from errors " << current_set.state_function().errors())
    ...
    LOGGING_PRINTLN("step_size = "<<step_size)
    ...
    LOGGING_PRINTLN_AT(1,"flow_step_model = "<<flow_step_model)

    // Compute the integration time model
    TimeStepType next_time=current_time+TimeStepType(step_size);
    LOGGING_PRINTLN_AT(1,"next_time = "<<next_time)
