...
class CombinedFunction
{
  ...
  public:
    ...
    ScalarFunction<P,ARG> operator[](SizeType i) const { ARIADNE_NOT_IMPLEMENTED; }
    template<class X> inline ElementType<C,X> operator() (const ElementType<D,X>& x) const {
        ElementType<D1,X> x1=project(x,range(0,_f1.argument_size()));
        ElementType<D2,X> x2=project(x,range(_f1.argument_size(),_f1.argument_size()+_f2.argument_size()));
        return join(_f1.evaluate(x1),_f2.evaluate(x2)); }
