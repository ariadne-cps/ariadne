    ...
    , public virtual DrawableInterface
{
  protected:
    ...
    friend GridTreePaving outer_approximation( const EuclideanCompactSetInterface& theSet, const Grid& theGrid, const Nat numSubdivInDim );
    friend GridTreePaving inner_approximation( const EuclideanOpenSetInterface& theSet, const Grid& theGrid, const Nat numSubdivInDim );
    ...
    , public GridTreeSubpaving
{
  ...
  public:
    ...
    Void adjoin_outer_approximation( const EuclideanCompactSetInterface& theSet, const Nat numSubdivInDim );
    ...
    Void adjoin_lower_approximation( const EuclideanOvertSetInterface& theSet, const Nat heightInDim, const Nat numSubdivInDim );
    ...
    Void adjoin_lower_approximation( const EuclideanOvertSetInterface& theSet, const ExactBoxType& bounding_box, const Nat numSubdivInDim );

    //! \brief Adjoin a lower approximation to a given set, computing to the given resolution
    //! \a numSubdivInDim -- defines, how many subdivisions in each dimension from the level of the
    //! zero cell we should make to get the proper cells for outer approximating \a theSet.
    //! A lower approximation comprises all cells intersecting a given set.
    Void adjoin_lower_approximation( const EuclideanLocatedSetInterface& theSet, const Nat numSubdivInDim );
    ...
    Void adjoin_inner_approximation( const EuclideanOpenSetInterface& theSet, const Nat heightInDim, const Nat numSubdivInDim );
    ...
    //! the given set.
    Void adjoin_inner_approximation( const EuclideanOpenSetInterface& theSet, const ExactBoxType& bounding_box, const Nat numSubdivInDim );
    ...
    //! approximating \a theSet. An inner approximation comprises all cells that are sub-cells of
    //! the given set.
    Void adjoin_inner_approximation( const EuclideanSetInterface& theSet, const Nat numSubdivInDim );
