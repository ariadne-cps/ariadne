    ...
    , public virtual DrawableInterface
{
  ...
  public:
    ...
    virtual ValidatedLowerKleenean _inside( const ExactBoxType& theBoxType  ) const override;

    //! \brief Tests if (the closure of) a grid set is disjoint from (the closure of) a box.
    virtual ValidatedLowerKleenean _separated( const ExactBoxType& theBoxType  ) const override;

    //! \brief Tests if a grid set overlaps (intersects the interior of) a box.
    virtual ValidatedLowerKleenean _overlaps( const ExactBoxType& theBoxType ) const override;
