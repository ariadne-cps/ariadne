    ...
    -> Pair<ValidatedKleenean,ExactPointType>
{
    ...
    if(decide(t<TERR)) {
        ...
        LOGGING_PRINTLN_AT(1,"dom="<<subdomain);
        ...
        for(SizeType i=0; i!=m; ++i) {
            ...
            LOGGING_PRINTLN_AT(2,"dom="<<subdomain);
            if(definitely(subdomain.is_empty())) { LOGGING_PRINTLN("Proved disjointness using box reduce"); return make_pair(false,ExactPointType()); }
        }
        LOGGING_PRINTLN_AT(1,"dom="<<subdomain);
