    ...
    -> Tuple< AX, Vector<AX>, Vector<AX> >
{
    ...
    LOGGING_PRINTLN("xl="<<xl<<" xu="<<xu<<" A="<<A<<" b="<<b<<" c="<<c);
    LOGGING_PRINTLN("x="<<x<<" y="<<y<<" z="<<zl<<" zu="<<zu);
...
//        std::cerr<<step<<": result="<<result<<"\n";
        ...
        if(result==LinearProgramStatus::DEGENERATE_FEASIBILITY) { LOGGING_PRINTLN("Degenerate"); return indeterminate; }
        if(decide(compute_mu(xl,xu, x,zl,zu)<THRESHOLD) ) { LOGGING_PRINTLN("Threshold"); return indeterminate; }
    }
    return indeterminate;
}
...
{
    LOGGING_SCOPE_CREATE;
    LOGGING_PRINTLN("x="<<x<<", y="<<y<<", zl="<<zl<<", zu="<<zu);
    ...
    S=adat(A,D.diagonal());
    LOGGING_PRINTLN("S="<<S<<", inverse(S)="<<inverse(S));
...
//    std::cerr<<"dy="<<dy<<", is_nan(dy)="<<is_nan(dy)<<"\n";
    ...
    LOGGING_PRINTLN("dx="<<dx<<" dy="<<dy<<" dzl="<<dzl<<" dzu="<<dzu);

    LOGGING_PRINTLN_AT(1,"A*dx="<<(A*dx)<<" AT*dy+dzl-dzu="<<(transpose(A)*dy+dzl-dzu)<<" Zl*dx+(Xa-Xl)*dzl="<<(Zl*dx+(Xa-Xl)*dzl)<<" -Zu*dx+(Xu-Xa)*dzu="<<(-(Zu*dx)+(Xu-Xa)*dzu));
    LOGGING_PRINTLN_AT(1,"A*dx-rx="<<(A*dx-rx)<<" AT*dy+dzl-dzu-ry="<<(transpose(A)*dy+dzl-dzu-ry)<<" Zl*dx+(Xa-Xl)*dzl-rzl="<<(Zl*dx+(Xa-Xl)*dzl-rzl)<<" -Zu*dx+(Xu-Xa)*dzu-rzu="<<(-(Zu*dx)+(Xu-Xa)*dzu-rzu));
    ...
    ny=(y-alphaz*dy);
    LOGGING_PRINTLN("alphax="<<alphax<<" nx="<<nx<<" alphaz="<<alphaz<<" ny="<<ny<<" nzl="<<nzl<<" nzu="<<nzu);

    x=nx; y=ny; zl=nzl; zu=nzu;
    LOGGING_PRINTLN("cx="<<dot(c,x)<<" yb="<<dot(y,b)<<" Ax-b="<<(A*x-b)<<" yA+(zl-zu)-c="<<(transpose(A)*y+(zl-zu)-c));
