        ...
        -> ApproximateScalarMultivariateFunction
{
    ...
    for (SizeType i=0; i!=n; ++i) {
        ARIADNE_DEBUG_ASSERT(likely(D[i].lower_bound()<D[i].upper_bound()));
        if (likely(D[i].lower_bound() < D[i].upper_bound())) {
...
#warning
    ...
    for (SizeType i=0; i!=D.dimension(); ++i) { xw.x[i] += 0.1; }
    for (SizeType j=0; j!=C.dimension(); ++j) { if (likely(C[j].lower_bound()<C[j].upper_bound())) { xw.w[j] += 0.1; } }
...
/*
    CONCLOG_PRINTLN_AT(3,"x="<<x<<", D="<<D<<", contains(D,x)="<<contains(D,x))
    CONCLOG_PRINTLN_AT(3,"w="<<w<<", C="<<C<<", contains(C,w)="<<contains(C,w))
*/
...
contains_feasible_point(ExactBoxType D, ValidatedVectorMultivariateFunction g, ExactBoxType C, ValidatedVectorType X) const
{
    ...
    CONCLOG_PRINTLN("D="<<D<<", g="<<g<<", C="<<C<<", X="<<X);
