...
RestrictedExpression<ValidatedTag,Real> make_restricted_expression(RealVariableDomainMap doms, RealExpression) {
    ARIADNE_NOT_IMPLEMENTED;
}
RestrictedExpression<ValidatedTag,Vector<Real>> make_restricted_expression(RealVariableDomainMap doms, Vector<RealExpression>) {
    ARIADNE_NOT_IMPLEMENTED;
}
}// namespace
...
RestrictedExpression<ValidatedTag,RealVector>::RestrictedExpression(VariablesBoxDomainType const& vdoms)
    : _vars(vdoms.variables()), _fp(ValidatedVectorMultivariateFunctionPatch::identity(vdoms.box()))
{
    assert(fdom.dimension()==f.argument_size());
    assert(f.argument_size()==vars.size());
}
...
auto RestrictedExpression<ValidatedTag,RealVector>::_join(RestrictedExpression<P,T> ep1, RestrictedExpression<P,T> ep2) -> RestrictedExpression<P,T> {
...
/*
template<class P, class... ARGS> auto FunctionPatch<P,RealScalar(ARGS...)>::operator() (InitializerList<AnyConstantOrVariablePatch> const& cvs) const -> RestrictedExpression<P,RealScalar> {
...
operator() (ConstantOrVariable<ARGS> const& ... cvs) const -> RestrictedExpression<P,RealScalar> {
    List<ConstantOrVariable<Real>> cvl=make_constant_or_variable_list(cvs...);
    return make_expression_patch(*this,cvl);
}

template<class P, class... ARGS> auto FunctionPatch<P,RealVector(ARGS...)>::operator() (InitializerList<AnyConstantOrVariablePatch> const& cvs) const -> RestrictedExpression<P,RealVector> {
    List<ConstantOrVariablePatch<Real>> cvl=make_constant_or_variable_patch_list(cvs);
    return make_expression_patch(*this,cvl);
}

template<class P, class... ARGS> auto FunctionPatch<P,RealVector(ARGS...)>::
operator() (ConstantOrVariable<ARGS> const& ... cvs) const -> RestrictedExpression<P,RealVector> {
    List<ConstantOrVariable<Real>> cvl=make_constant_or_variable_list(cvs...);
    return make_expression_patch(*this,cvl);
}

template<class P, class... ARGS> auto FunctionPatch<P,RealScalar(ARGS...)>::
operator() (RestrictedExpression<P,ARGS...> const& ep) const -> RestrictedExpression<P,RealScalar> {
    static_assert (Same<Tuple<ARGS...>,Tuple<RealVector>>);
    return RestrictedExpression<P,RealScalar>(ep._vars,compose(*this,ep._fp));
}

template<class P, class... ARGS> auto FunctionPatch<P,RealVector(ARGS...)>::
operator() (RestrictedExpression<P,ARGS...> const& ep) const -> RestrictedExpression<P,RealVector> {
    static_assert (Same<Tuple<ARGS...>,Tuple<RealVector>>);
    return RestrictedExpression<P,RealVector>(ep._vars,compose(*this,ep._fp));
}
*/
