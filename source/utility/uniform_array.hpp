...
template<class T, class TUP> concept MakeableFromTuple = requires (TUP const& tup) {
    std::make_from_tuple<T>(tup);
};
...
template<class T, class... PRS> requires MakeableFromTuple<CharacteristicsType<T>,Tuple<PRS...>> CharacteristicsType<T> make_characteristics(Tuple<PRS...> const& tup) {
    return std::make_from_tuple<CharacteristicsType<T>>(tup); }

template<class T, class... PRS> requires Constructible<T,PRS...> T make_from_characteristics(PRS... prs) {
    return T(prs...); }
template<class T, class... PRS> requires MakeableFromTuple<T,Tuple<PRS...>> T make_from_characteristics(Tuple<PRS...> const& tup) {
...
template<class T, class... PRS> requires Constructible<T,Int,PRS...> T make_zero_from_characteristics(PRS... prs) {
    return T(0,prs...); }
template<class T, class... PRS> requires MakeableFromTuple<T,Tuple<PRS...>> T make_zero_from_characteristics(Tuple<PRS...> const& tup) {
    return nul(std::make_from_tuple<T>(tup)); }
    ...
    : private PR
{
  ...
  public:
    ...
    UniformArray(SizeType n, G const& g, Tuple<PRS...> prs)
            : PR(get_characteristics(std::make_from_tuple<T>(prs))), _size(n), _ptr(uninitialized_new(_size)) {
        this->_uninitialized_generate(g); }
