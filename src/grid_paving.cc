...
/********************************************GridPavingCursor***************************************/
	...
	string GridPavingCursor::to_string() const{
...
/****************************************GridPavingConstIterator************************************/
	...
	void GridPavingConstIterator::find_next_enabled_leaf() {
	...
	bool GridPavingConstIterator::navigate_to(bool firstLast){
...
/*********************************************GridPavingCell***********************************************/
	...
	uint GridPavingCell::smallest_primary_cell( const dimension_type dimensions, const Box& theBox ) {
		int leftBottomCorner = 0, rightTopCorner = 1;
		uint height = 0;
		//The zero level coordinates are known, so we need to iterate only for higher level primary cells
		do{
	...
	Box GridPavingCell::grid_box_to_space(const Box & theGridBox, const Grid& theGrid ){
	...
	Box GridPavingCell::compute_box(const Grid& theGrid, const uint theHeight, const BinaryWord& theWord){
	 ...
	 BinaryWord GridPavingCell::primary_cell_path( const uint dimensions, const uint topPCellHeight, const uint bottomPCellHeight) {
	...
	string GridPavingCell::to_string() const{
...
/********************************************GridSubPaving*****************************************/
	...
	void GridSubPaving::subdivide( Float theMaxCellWidth ) {
		...
		const Box& theRootCellBox = _theGridPavingCell.box();
        ...
        GridSubPaving::operator ListSet<Box>() const {
		...
		for (GridSubPaving::const_iterator it = this->begin(), end = this->end(); it != end; it++ ) {
	...
	GridSubPaving::operator GridCellListSet() const {
	...
	/*! \brief Serialize the current object state, this is mostly needed for testing */
	string GridSubPaving::to_string() const{
...
/*********************************************GridPaving*********************************************/
	...
	BinaryTreeNode* GridPaving::align_with_cell( const GridPavingCell& theCell, const bool stop_on_enabled, bool & has_stopped ) {
		...
		if( thisPavingPCellHight > otherPavingPCellHight ){
			...
			BinaryWord primaryCellPath = GridPavingCell::primary_cell_path( this->cell().grid().dimension(), thisPavingPCellHight, otherPavingPCellHight );
			...
			if( thisPavingPCellHight < otherPavingPCellHight ) {
				...
				//1. Compute the path 
				BinaryWord primaryCellPath = GridPavingCell::primary_cell_path( this->cell().grid().dimension(), otherPavingPCellHight, thisPavingPCellHight );
				...
				this->_theGridPavingCell = GridPavingCell( this->_theGridPavingCell.grid(), otherPavingPCellHight, BinaryWord() );
			} else {
	...
	template<class Set> void GridPaving::adjoin_outer_approximation( BinaryTreeNode * pBinaryTreeNode, const uint primary_cell_height,
												const uint max_mince_depth,  const Set& theSet, BinaryWord * pPath ){
		//Compute the cell correspomding to the current node
		GridPavingCell theCurrentCell( GridSubPaving::_theGridPavingCell.grid(), primary_cell_height, *pPath );
	...
	template<class Set> inline void GridPaving::adjoin_outer_approximation( const Set& theSet, const uint depth ) {
		...
		//     return us it's box in the grid theGrid but not in the original space
                ...
                //GridPaving theGridPavingOfSetOuterBox( theGrid, GridBlock( Grid( theGrid.dimension(), R(1.0) ), theSetGridBlock.lattice_set() ).bounding_box() );
		GridPaving theGridPavingOfSetOuterBox;

		GridPavingCell theSetBoxCell( theGridPavingOfSetOuterBox.cell() );
	...
	void GridPaving::restrict( const GridSubPaving& theOtherSubPaving ) {
		throw NotImplemented(__PRETTY_FUNCTION__);
	}
	
	void GridPaving::remove( const GridPavingCell& theCell ) {
		throw NotImplemented(__PRETTY_FUNCTION__);
	}
	
	void GridPaving::remove( const GridSubPaving& theOtherSubPaving ) {
		throw NotImplemented(__PRETTY_FUNCTION__);
	}
