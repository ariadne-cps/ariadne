...
void TestDiscretiser<R>::test() const
{
  ...
  GridCellListSet<R> evolve_set = discretiser.upper_evolve(vdp,initial_cell,time);
  cout << "evolve_set=" << evolve_set << endl;
  GridCellListSet<R> reach_set = discretiser.upper_reach(vdp,initial_cell,time);
  cout << "reach_set=" << reach_set << endl;
  
  // Print the intial, evolve and reach sets
  epsfstream eps;
  eps.open("test_discretiser-vdp.eps",eps_bounding_box);
  eps << line_style(true);
  eps << fill_colour(cyan) << reach_set;
  eps << fill_colour(yellow) << evolve_set;
  eps << fill_colour(blue) << initial_cell;
  eps.close();
