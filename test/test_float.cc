...
template<class R> class TestFloat {
 public:
  TestFloat() {
	...
	{
		// Set up some variables
		R f1(1.25); R f2(2.25); R f3; R f4;
		
		// Rounding
		f3=next_down(f1);
		f4=next_up(f1);
		cout << f3 << " < " << f1 << " < " << f4 << endl;
		ARIADNE_TEST_ASSERT(f3<f1); 
		ARIADNE_TEST_ASSERT(f4>f1);
	}
	...
	void test_div()
	{
		...
		f5=div_approx(f1,f2);
		cout << f3 << " <= " << f1 << " / " << f2 << " <= " << f4 << endl;
		ARIADNE_TEST_ASSERT(f3<f4); 
		ARIADNE_TEST_ASSERT(f3<=f5); ARIADNE_TEST_ASSERT(f4>=f5);
	...
	{
		// Set up some variables
		R f1(1.25); R f2(-3.25); R f3(-2); R f4; R f5;
    ...
    ARIADNE_TEST_CALL(test_min());
    ARIADNE_TEST_CALL(test_max());
    ARIADNE_TEST_CALL(test_abs());
    ...
    ARIADNE_TEST_CALL(test_floor_ceil());
    ARIADNE_TEST_CALL(test_integer());
    ARIADNE_TEST_CALL(test_interval());
...
int main() {
  ...
  cout << "Testing Float64..." << endl << endl;
  TestFloat<Float64>().test();
  cout << endl;
#endif

#ifdef ENABLE_FLOATMP
  cout << "Testing FloatMP..." << endl << endl;
  TestFloat<FloatMP>().test();
  cout << endl;
#endif
