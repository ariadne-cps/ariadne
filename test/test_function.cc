...
void TestFunction::test_concept()
{
    ...
    ARIADNE_TEST_EQUAL(aff.b(),b);
    ARIADNE_TEST_EQUAL(aff.A(),A);
    
    Vector<Interval> ib(1); Matrix<Interval> iA(1,1);
    ib[0]=Interval(1.0)/Interval(3.0);
    iA[0][0]=1.0;
    VectorAffineFunction iaff(iA,ib);
    ARIADNE_TEST_EQUAL(iaff.b(),ib);
    ARIADNE_TEST_EQUAL(iaff.A(),iA);
    ...
    ARIADNE_TEST_EQUAL(raff.b(),rb);
    ARIADNE_TEST_EQUAL(raff.A(),rA);
#endif // HAVE_RATIONAL
