...
#include "geometry/grid_approximation.code.h"
...
/*
* Combines the string output matching the output of the to_string method of GridPavingCell
*/
...
/*
* Combines the string output matching the output of the to_string method of GridSubPaving
*/
...
template<class R> void test_grid_paving_cursor(){
	string expected_result;
	
	//Allocate the Grid
	Grid<R> theGrid( Box<R>( "[0,1]x[0,1]x[0,1]" ), LatticeBlock( "[1,5]x[-1,3]x[-6,-2]" ) );
	...
	//Create the GridSubPaving
	GridSubPaving<R> theGridSubPavingSmall( theGrid, theHeight, thePathToSubPavingRoot, pRootTreeNode->left_node()->right_node() );
	//Create the Cursor for the GridSubPaving
	GridPavingCursor<R> theGSPCursorSmall( &theGridSubPavingSmall );

	//Create the GridSubPaving
	GridSubPaving<R> theGridSubPavingLarge( theGrid, theHeight, BinaryWord(), pRootTreeNode );
	//Create the Cursor for the GridSubPaving
	GridPavingCursor<R> theGSPCursorLarge( &theGridSubPavingLarge );
	...
	const GridSubPaving<R> tmpSubPaving = *theGSPCursorLarge;
	expected_result = createSubPavingOutput("origin=[-0.25,0.25,1.5], lengths=[0.25,0.25,0.25]","2","01","[-0.5,0]x[0.5,1]x[1.25,2.25]","?1-01+0");
	...
	GridPaving<R> theGridPaving( theGrid, theHeight, pRootTreeNode );
	//Create the Cursor for the GridSubPaving
	GridPavingCursor<R> theGPCursor( &theGridPaving );
...
template<class R> void test_iterator( const string expected_result[], const GridSubPaving<R> & theGridSubPaving, const int expected_number_elements ){
	int elements_count = 0;
	for (typename GridSubPaving<R>::const_iterator it = theGridSubPaving.begin(), end = theGridSubPaving.end(); it != end; it++, elements_count++) {
...
template<class R> void test_grid_paving_const_iterator(){
	string expected_result[8];
	
	//Allocate the Grid
	Grid<R> theGrid(Box<R>("[0,1]x[0,1]"),LatticeBlock("[1,5]x[-1,3]"));
	...
	test_iterator<R>( expected_result, theGridSubPavingLarge, 1 );
	...
	test_iterator<R>( expected_result, theGridSubPavingLarge, 2 );
	...
	test_iterator<R>( expected_result, theGridSubPavingLarge, 4 );
	...
	test_iterator<R>( expected_result, theGridSubPavingLarge, 8 );
	...
	test_iterator<R>( expected_result_tmp, theGridSubPavingLarge, 4 );
	...
	test_iterator<R>( expected_result_tmp, theGridSubPavingLarge, 3 );
	...
	typename GridSubPaving<R>::const_iterator it = theGridSubPavingLarge.begin();
	const GridPavingCursor<R> theGPCursor = it.cursor();
	ARIADNE_TEST_EQUAL(theGPCursor.is_leaf(), true);
	ARIADNE_TEST_EQUAL(theGPCursor.is_root(), false);
	ARIADNE_TEST_EQUAL(theGPCursor.is_enabled(), true);
	ARIADNE_TEST_EQUAL(theGPCursor.is_disabled(), false);
...
template<class R> void test_grid_sub_paving(){
	string expected_result;
	
	//Allocate the Grid, one Dimension
	const Grid<R> theOneDimGrid(1, 0.5);
	...
	//Create the GridSubPaving
	GridSubPaving<R> theGridSPOneDim( theOneDimGrid, theHeight, thePathToSubPavingRoot, pRootTreeNode );
	
	//
	print_title("Test Mincing operations of GridSubPaving on the one dimensional Grid");
	...
	test_iterator<R>( expected_result_arr, theGridSPOneDim, 4 );
	...
	test_iterator<R>( expected_result_arr_tmp, theGridSPOneDim, 2 );
	...
	// !!!
	print_title("Test Mincing operations of GridSubPaving on the two dimensional Grid");
	//Allocate the Grid, one Dimension
	const Grid<R> theTwoDimGrid(Box<R>("[0,1]x[0,2]"),LatticeBlock("[1,5]x[-1,3]"));

	//Create the GridSubPaving
	GridSubPaving<R> theGridSPTwoDim( theTwoDimGrid, theHeight, thePathToSubPavingRoot, pRootTreeNode );
	...
	test_iterator<R>( expected_result_arr, theGridSPTwoDim, 4 );
...
template<class R> void test_grid_paving(){
	string expected_result;
	
	// !!!
	print_title("Test allocation of a trivial GridPaving");
	GridPaving<R> * pTrivialPaving = new GridPaving<R>(4, true);
	...
	// !!!
	print_title("Test GridPaving copy constructor");
	GridPaving<R> theTrivialPaving( ( *pTrivialPaving ) );
	...
	// !!!
	print_title("Test GridPaving (Grid, Box) constructor");
	...
	GridPaving<R> theTwoDimPaving( theTwoDimGrid, Box<R>("[0,1.5]x[-1.5,3.5]") );
	TEST_TO_STRING_RESULT("The resulting GridPaving: ", expected_result, theTwoDimPaving );

	// !!!
	print_title("Test GridPaving (Grid, Height, BooleanArray, BooleanArray) constructor");
	...
	GridPaving<R> theTwoDimTreePaving( theTwoDimGrid, 2, theTree, theEnabledLeafs );
	TEST_TO_STRING_RESULT("The resulting GridPaving: ", expected_result, theTwoDimTreePaving );
	
}

template<class R> void test_grid_paving_cell(){
	string expected_result;

	// !!!
	print_title("Test the static methods of the GridPavingCell");
	BinaryWord theBinaryPath;
	
	expected_result = "e";
	theBinaryPath = GridPavingCell<R>::primary_cell_path( 1, 0, 0 );
	TEST_BINARY_WORD( "Dimension: 1, topCellHeight: 0, bottomCellHeight: 0", expected_result , theBinaryPath );
	
	expected_result = "1";
	theBinaryPath = GridPavingCell<R>::primary_cell_path( 1, 1, 0 );
	TEST_BINARY_WORD( "Dimension: 1, topCellHeight: 1, bottomCellHeight: 0", expected_result , theBinaryPath );
	
	expected_result = "01";
	theBinaryPath = GridPavingCell<R>::primary_cell_path( 1, 2, 0 );
	TEST_BINARY_WORD( "Dimension: 1, topCellHeight: 2, bottomCellHeight: 0", expected_result , theBinaryPath );
	
	expected_result = "0";
	theBinaryPath = GridPavingCell<R>::primary_cell_path( 1, 2, 1 );
	TEST_BINARY_WORD( "Dimension: 1, topCellHeight: 2, bottomCellHeight: 1", expected_result , theBinaryPath );
	
	expected_result = "e";
	theBinaryPath = GridPavingCell<R>::primary_cell_path( 1, 2, 2 );
	TEST_BINARY_WORD( "Dimension: 1, topCellHeight: 2, bottomCellHeight: 2", expected_result , theBinaryPath );
}

template<class R> void test_adjoin_operation_one(){
	string expected_result;
	
	//Allocate a trivial Grid
	Grid<R> theGrid(2, 1.0);
	
	// !!!
	print_title("Test adjoining a GridPavingCell to the GridPaving");
	//Define the GridPavingCell that is rooted to a high primary cell
	...
	theHigherCellPath.push_back(true);
	theHigherCellPath.push_back(false);
	GridPavingCell<R> theHigherLevelCell( theGrid, theHigherCellHeight, theHigherCellPath );

	print_comment("The GridPavingCell with the primary root cell height = 2");
	expected_result = createPavingCellOutput("lengths=[1,1]","2","1010","[2,3]x[-1,0]");
	TEST_TO_STRING_RESULT("The initial GridPavingCell: ", expected_result, theHigherLevelCell );
	
	//Define the higth of the primary root cell.
	//Create the GridPaving with the box is related to the grid, but not to the original space
	GridPaving<R> theOneCellPaving( theGrid, true );
	expected_result = createSubPavingOutput("lengths=[1,1]", "0", "e", "[0,1]x[0,1]", "+0");
	print_comment("The GridPaving with the primary root cell height = 0");
	TEST_TO_STRING_RESULT("The initial GridPaving: ", expected_result, theOneCellPaving );
	...
	TEST_TO_STRING_RESULT("The GridPaving after adding the cell: ", expected_result, theOneCellPaving );
}

template<class R> void test_adjoin_operation_two(){
	string expected_result;
	...
	// !!!
	print_title("Test adjoining a GridPavingCell to the GridPaving");
	//Define the GridPavingCell that is rooted to the lower primary cell
	const int theLowerCellHeight = 1;
	BinaryWord theLowerCellPath;
	theLowerCellPath.push_back(true);
	theLowerCellPath.push_back(true);
	GridPavingCell<R> theLowerLevelCell( theGrid, theLowerCellHeight, theLowerCellPath );

	print_comment("The GridPavingCell with the primary root cell height = 1");
	expected_result = createPavingCellOutput("lengths=[1,1]","1","11","[0,1]x[0,1]");
	TEST_TO_STRING_RESULT("The initial GridPavingCell: ", expected_result, theLowerLevelCell );
	...
	//Create the GridPaving with the box is related to the grid, but not to the original space
	GridPaving<R> theOneCellPaving( theGrid, theHeight, pRootTreeNode );
	expected_result = createSubPavingOutput("lengths=[1,1]", "2", "e", "[-1,3]x[-1,3]", "?1-01?1?1-01?1+01-01-0");
	print_comment("The GridPaving with the primary root cell height = 2");
	TEST_TO_STRING_RESULT("The initial GridPaving: ", expected_result, theOneCellPaving );
...
template<class R> void test_adjoin_operation_three(){
	string expected_result;
	...
	// !!!
	print_title("Test adjoining a GridPavingCell to the GridPaving");
	//Define the GridPavingCell that is rooted to the same primary cell
...
template<class R> void test_adjoin_outer_approximation_operation(){
	string expected_result;
	
	//Allocate a trivial Grid
	Grid<R> theTrivialGrid(2, 1.0);
	
	// !!!
	print_title("Test adjoining_outer_approximation a SetInterface to the GridPaving");
	RectangularSet<R> initialRectangle( Box<R>("[-0.5,1.5]x[-0.3,1.0]") );
	...
	//Create the GridPaving with the box is related to the grid, but not to the original space
	GridPaving<R> theOneCellPaving( theTrivialGrid, theHeight, pRootTreeNode );
	expected_result = createSubPavingOutput("lengths=[1,1]", "2", "e", "[-1,3]x[-1,3]", "?1-01?1?1-01?1+01-01-0");
	print_comment("The GridPaving with the primary root cell height = 2");
	TEST_TO_STRING_RESULT("The initial GridPaving: ", expected_result, theOneCellPaving );
	
	theOneCellPaving.adjoin_outer_approximation( static_cast<const SetInterface< Box<R> >&>(initialRectangle), 2 );
	...
	print_comment("Recombined GridPaving after adding the cell: ");
	...
	test_iterator<R>( expected_result_arr, theOneCellPaving, 16 );
	
	// !!!
	print_title("Create an outer_approximation of the rectangle on the scaling grid and get the GridPaving");
	Grid<R> theScalingGrid(2, 2.0);
	GridPaving<R> theOuterApproxGridPaving = outer_approximation( theScalingGrid, static_cast<SetInterface< Box<R> >&>(initialRectangle), 2 );
	//IVAN S. ZAPREEV
	...
	theOuterApproxGridPaving.recombine();
	...
	test_iterator<R>( expected_result_arr, theOuterApproxGridPaving, 5 );
}
...
int main() {
	...
	test_grid_paving_cursor<Flt>();

	test_grid_paving_const_iterator<Flt>();

	test_grid_paving_cell<Flt>();
	
	test_grid_sub_paving<Flt>();
	
	test_grid_paving<Flt>();
	
	test_adjoin_operation_one<Flt>();
	
	test_adjoin_operation_two<Flt>();
	
	test_adjoin_operation_three<Flt>();

	test_adjoin_outer_approximation_operation<Flt>();
