...
public:
  ...
  Void test_unconstrained_optimisation()
  {
    ...
    EffectiveScalarMultivariateFunction f(
        x0s * (12 + x0s * (Decimal(6.3) + x0s)) + 6 * x[1] * (x[1] - x[0]));
  ...
  Void test_equality_constrained_optimisation()
  {
    ...
    Real                                a(1.5);
    Real                                b(0.25);
    EffectiveVectorMultivariateFunction g = {a + x[0] + 2 * x[1] +
                                             b * x[0] * x[1]};
    ARIADNE_TEST_PRINT(g);
  ...
  Void test_constrained_optimisation()
  {
    ...
    EffectiveScalarMultivariateFunction f =
        ...
        x[2];
    ARIADNE_TEST_PRINT(f);
    // EffectiveVectorMultivariateFunction g( (x[0]-1, x[0]+x[1]*x[1],
    // x[1]*x[1]) );
  ...
  Void test_linear_feasibility()
  {
    ...
    EffectiveVectorMultivariateFunction g =
        EffectiveVectorMultivariateFunction(1u, 2 * x[0] + x[1]);
    ARIADNE_TEST_PRINT(g);
  ...
  Void test_feasibility_check()
  {
    EffectiveVectorMultivariateFunction x =
        EffectiveVectorMultivariateFunction::identity(2);
  ...
  Void test_1()
  {
    ...
    EffectiveVectorMultivariateFunction g = {log(x[0]) + Decimal(1),
                                             cos(x[0] + x[1])};
    ExactBoxType C = ExactBoxType{{0.0, inf}, {0.0, inf}};
  ...
  Void test_5()
  {
    ...
    EffectiveVectorMultivariateFunction g = {
        x[0] + x[1] - 1, exp(x[0]), x[1] * x[0], x[1] * x[1] + x[0], sin(x[1])};
    ExactBoxType C =
        ExactBoxType{{0, 100}, {0, 100}, {0, 100}, {0, 10}, {0, 10}};
  ...
  Void test_6()
  {
    ...
    EffectiveScalarMultivariateFunction f(exp(x[0] * x[1]) - (x[1] * x[2]) +
                                          10 * (x[3] * x[3]));
    ...
    ExactBoxType test_D = ExactBoxType{
        {-0.01, 6.01}, {-0.01, 6.01}, {-0.01, 6.01}, {-0.01, 6.01}};
  ...
  Void test_7()
  {
    ...
    ExactBoxType                        D = ExactBoxType{{-10, 10}, {1, 10}};
    EffectiveVectorMultivariateFunction g = {
        x[0] * x[1], x[1] * x[1], log(x[1]), Decimal(3) * x[0] + x[1],
        sin(x[1]),   sqrt(x[1])};
    ExactBoxType C = ExactBoxType{{-10, 100}, {0, 100}, {-1, 2},
                                  {-10, 10},  {-3, 3},  {0, 10}};
    ...
    ExactBoxType test_C =
        ExactBoxType{{-10.01, 100.01}, {-0.01, 100.01}, {-1.01, 2.01},
                     {-10.01, 10.01},  {-3.01, 3.01},   {-0.01, 10.01}};
    ARIADNE_TEST_BINARY_PREDICATE(element, x_optimal, test_D);
    ARIADNE_TEST_BINARY_PREDICATE(element, g(x_optimal), test_C);
    FloatDPValue required_accuracy(1e-6);
    // ARIADNE_TEST_LESS(norm(x_optimal),required_accuracy);
  ...
  Void test_8()
  {
    List<EffectiveScalarMultivariateFunction> x =
        EffectiveScalarMultivariateFunction::coordinates(2);
    EffectiveScalarMultivariateFunction f(x[0] * (sin(x[0]) + cos(x[1])) -
                                          exp(-x[1]));
    ExactBoxType                        D = ExactBoxType{{-10, 10}, {0, 10}};
    EffectiveVectorMultivariateFunction g = {x[0] * x[0], x[1] * x[1],
                                             x[0] * x[1]};
    ...
    ExactBoxType test_C =
        ExactBoxType{{-0.01, 10.01}, {-0.01, 100.01}, {-10.01, 8.01}};
    ARIADNE_TEST_BINARY_PREDICATE(element, x_optimal, test_D);
    ARIADNE_TEST_BINARY_PREDICATE(element, g(x_optimal), test_C);
    FloatDPValue required_accuracy(1e-6);
    // ARIADNE_TEST_LESS(norm(x_optimal),required_accuracy);
  ...
  Void test_10()
  {
    ...
    ExactBoxType test_D =
        ExactBoxType{{-0.01, 10.01}, {-0.01, 10.01}, {-0.01, 10.01}};
  ...
  Void temp_test()
  {
    ...
    unsigned                                  nVars = 2;
    List<EffectiveScalarMultivariateFunction> x =
        EffectiveScalarMultivariateFunction::coordinates(nVars);
    ...
    EffectiveScalarMultivariateFunction f(pow(x0 + 1, 2) + 1);
    ARIADNE_TEST_PRINT(f);
    EffectiveVectorMultivariateFunction g = {13 * x0 * x1 + 76 * x1 * x0 +
                                             46 * x1 * x1};
    ARIADNE_TEST_PRINT(g);
