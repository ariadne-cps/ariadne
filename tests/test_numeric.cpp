...
template<class OP, class... XS> struct UnsafeTypedef;
template<class OP, class... XS> using UnsafeType = typename UnsafeTypedef<OP,XS...>::Type;
template<class OP, class X> struct UnsafeTypedef<OP,X> { typedef decltype(declval<OP>()(declval<X>())) Type; };
template<class OP, class X1, class X2> struct UnsafeTypedef<OP,X1,X2> { typedef decltype(declval<OP>()(declval<X1>(),declval<X2>())) Type; };
template<class X> struct UnsafeTypedef<Plus,X> { typedef decltype(operator+(declval<X>())) Type; };
template<class X> struct UnsafeTypedef<Minus,X> { typedef decltype(-(declval<X>())) Type; };
template<class X1, class X2> struct UnsafeTypedef<Plus,X1,X2> { typedef decltype(operator+(declval<X1>(),declval<X2>())) Type; };
template<class X1, class X2> struct UnsafeTypedef<Minus,X1,X2> { typedef decltype(operator-(declval<X1>(),declval<X2>())) Type; };
template<class X1, class X2> struct UnsafeTypedef<Times,X1,X2> { typedef decltype(operator*(declval<X1>(),declval<X2>())) Type; };
template<class X1, class X2> struct UnsafeTypedef<Divides,X1,X2> { typedef decltype(operator/(declval<X1>(),declval<X2>())) Type; };
template<class X1, class X2> struct UnsafeTypedef<Add,X1,X2> { typedef decltype(add(declval<X1>(),declval<X2>())) Type; };
template<class X1, class X2> struct UnsafeTypedef<Sub,X1,X2> { typedef decltype(sub(declval<X1>(),declval<X2>())) Type; };
template<class X1, class X2> struct UnsafeTypedef<Mul,X1,X2> { typedef decltype(mul(declval<X1>(),declval<X2>())) Type; };
template<class X1, class X2> struct UnsafeTypedef<Div,X1,X2> { typedef decltype(div(declval<X1>(),declval<X2>())) Type; };

template<class X1, class X2=X1> using UnsafeArithmeticType = UnsafeType<Plus,UnsafeType<Times,X1,X2>,UnsafeType<Times,X1,X2>>;
...
class CheckNumeric
{
  ...
  private:
    ...
    struct ExpectedDifference { template<class A1, class A2> using Type = UnsafeType<Plus,A1,UnsafeType<Neg,A2>>; };
    struct ExpectedQuotient { template<class A1, class A2> using Type = UnsafeType<Times,A1,UnsafeType<Rec,A2>>; };
    struct ExpectedLess { template<class A1, class A2> using Type = UnsafeType<Sgn,UnsafeType<Minus,A1,A2>>; };
    struct ExpectedEqual { template<class A1, class A2> using Type = UnsafeType<Equal,A1,A2>; };
